...
class Query implements \ArrayAccess, \Iterator {
	 ...
	 * @param string  $pk	自增主键名，默认为id
	 * @return array 查询结果集
	 */
	public function find($id = 0, $pk = 'id') {
		if ($id > 0) {
			$this->where($pk, $id);			
		}
	 ...
	 * @return integer  影响行数
	 */
	public function delete($ids = [], $pk = 'id') {		
		if (!empty($ids)) {
			if (is_numeric($ids)) {
				$this->where($pk, $ids);				
			} else {
	 ...
	 * @param  string  $pk           自增主键名
	 * @return integer 影响行数或自增ID
	 */
	public function insert(array $data = [], $getLastInsID = false, $replace = false, $pk = null) {
		$options = $this->parseExpress();
		$data = array_merge($options['data'], $data);		
	 ...
	 * @return integer 影响行数或自增ID
	 */
	public function replace(array $data = [], $getLastInsID = false, $pk = null) {		
		return $this->insert($data, $getLastInsID, true, $pk);
	}	
	/**
	 * 插入返回自增ID
	 * @param  array   $data         插入数据
	 ...
	 * @return integer 影响行数或自增ID
	 */
	public function insertGetId(array $data = [], $pk = null) {
		return $this->insert($data, true, false, $pk);
	}
	/**
	 * 批量插入数据
